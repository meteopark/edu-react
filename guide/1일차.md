...
VsCode ( 강사님께서는 Webstorm을 추천해 주셨으나 난 Vscode가 미리 설치되어 있어서 Vscode를 사용 )
...
이 중 수업시간에 다룬 내용 중 중요한 부분을 정리 했다.
...
```
## `props`
컴포넌트와 컴포넌트 사이에 data를 전달할때 사용 된다.
```js
const App = () => {
  return (
...
위에서 언급한 대로 React는 `단방향 바인드` 이기 때문에 `자식의 자식의 ... 자식` 등으로 넘긴 데이터에 대해서는 임의적으로 변경 시키면 안된다.
```js
...
하지만 이 부분도 `자식의 자식의 ... 자식`이 되버린다면 데이터 변경 시 불편한 사항이 발생된다.<br>
이를 해결하기 위해 다음강의 때는 `Redux` 라는 개념을 배울 것이다.
